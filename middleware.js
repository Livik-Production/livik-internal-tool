...
export function middleware(req) {
  ...
  if (isDashboard || (isApi && !isAuthApi && !isPublicApi)) {
    ...
    const isFullAdmin = roleName === 'ADMIN' || roleName === 'SUPER_ADMIN';
    const hasAllAccess = isFullAdmin || rights.includes('ALL_ACCESS');
    ...
    if (!hasAllAccess) {
      ...
      if (isDashboard) {
        ...
        for (const [route, rightKeyword] of Object.entries(routeMap)) {
          if (pathname.startsWith(route)) {
            const hasRight = rights.some((r) =>
              r.toLowerCase().includes(rightKeyword)
            );
            if (!hasRight) {
      ...
      if (isApi) {
        ...
        for (const [route, rightKeyword] of Object.entries(apiRouteMap)) {
          if (pathname.startsWith(route)) {
            const hasRight = rights.some((r) =>
              r.toLowerCase().includes(rightKeyword)
            );
