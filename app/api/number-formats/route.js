        ...
        if (mod === 'employee' && nextNumber) {
          ...
          if (!isNaN(nextVal)) {
            try {
              ...
              await prisma.$executeRawUnsafe(`SELECT setval('${seqName}', ${nextVal - 1}, true)`);
            } catch (seqErr) {
        ...
        if (!oldS) {
          if (mod === 'invoice') {
            ...
            const defaultKeywords = {
              Laptop: 'LAP', Mobile: 'MB', TV: 'TV', Keyboard: 'KB',
              Monitor: 'MN', Mouse: 'MS', Printer: 'PR', Tablet: 'TB',
              Chair: 'CHR', Table: 'TBL', Camera: 'CAM', Other: 'OTH',
            };
        ...
        if (mod === 'employee') {
          ...
          await prisma.$transaction(async (tx) => {
            ...
            try {
              ...
              // Format to new IDs
              for (const inv of invoices) {
                let seq = extractSequence(inv.invoiceNumber, oldS.prefix, oldS.suffix);
                while (usedSequences.has(seq)) {
                  seq++;
                }
                usedSequences.add(seq);
