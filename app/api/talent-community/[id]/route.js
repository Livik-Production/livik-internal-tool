...
import { getTalentCommunityById, deleteTalentCommunity } from '../../../../lib/talentCommunityService.js';
...
export async function GET(req, { params }) {
  try {
    ...
    return addCorsHeaders(NextResponse.json({ error: 'Failed to fetch talent community entry' }, { status: 500 }));
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    return addCorsHeaders(NextResponse.json({ message: 'Talent community entry deleted successfully' }));
  } catch (error) {
    console.error(`DELETE /api/talent-community/[id] error:`, error);
    return addCorsHeaders(NextResponse.json({ error: 'Failed to delete talent community entry' }, { status: 500 }));
  }
}
