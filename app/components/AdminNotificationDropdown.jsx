...
import { Clock, CheckCircle2, AlertCircle, ChevronRight, LayoutGrid, Users, FileText, Settings, Bell } from 'lucide-react';
...
const getModuleIcon = (moduleName) => {
  switch (moduleName) {
    ...
    case 'Payroll': return <FileText size={16} />;
    default: return <LayoutGrid size={16} />;
  }
};

export default function AdminNotificationDropdown({ notifications, onClose, onMarkAllRead, onViewAll, onNotificationClick }) {
  ...
  const modules = Object.keys(groupedNotifications).sort((a, b) => groupedNotifications[b].unread - groupedNotifications[a].unread);

  const activeNotifications = hoveredModule ? groupedNotifications[hoveredModule]?.items || [] : [];

  return (
    <div className="relative flex animate-dashboard-reveal origin-top-right">
      
      {/* Main Dropdown (Modules List) */}
      <div className="w-[280px] bg-white border border-gray-100 rounded-[24px] shadow-2xl z-50 relative">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${unread > 0 ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
                        {getModuleIcon(moduleName)}
                      </div>
                      <div>
                        <h4 className="text-sm font-semibold text-gray-800">{moduleName}</h4>
                        <p className="text-[10px] font-medium text-gray-400 mt-0.5">{total} notification{total !== 1 ? 's' : ''}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <ChevronRight size={16} className={`transition-colors ${hoveredModule === moduleName ? 'text-blue-500' : 'text-gray-300'}`} />
                    </div>
                  </div>

                  {/* Side Flyout Panel aligned perfectly with this module */}
                  {hoveredModule === moduleName && activeNotifications.length > 0 && (
                    <div 
                      className="absolute right-[100%] top-0 w-[380px] bg-white border border-gray-100 rounded-[24px] shadow-2xl overflow-hidden z-50 transition-all duration-300 opacity-100 translate-x-0"
                    ...
                    >
                      <div className="p-4 pb-3 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                        <h3 className="text-md font-bold text-gray-900 flex items-center gap-2">
                          <span className="text-blue-500">{getModuleIcon(hoveredModule)}</span>
                      ...
                      <div className="max-h-[360px] overflow-y-auto no-scrollbar py-2">
                        {activeNotifications.map((notif, nIdx) => (
                          <div key={nIdx} onClick={() => onNotificationClick && onNotificationClick(notif)} className="px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer group border-b border-gray-50 last:border-0 relative">
                            <div className="flex gap-4 items-start">
                              <div className="relative">
                                {notif.avatar ? (
                                  <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-100">
                                    <Image src={notif.avatar} alt="Avatar" width={40} height={40} className="object-cover" />
                                  </div>
                                ) : (
