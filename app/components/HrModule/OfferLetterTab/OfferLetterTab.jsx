...
const OfferLetterTab = ({ isViewOnly = false }) => {
  ...
  const handleViewLetter = async () => {
    if (!selectedEmployee) {
      showAlert('Selection Required', 'Please select an employee first.', 'warning');
      return;
    }
    if (!selectedLetterType) {
      showAlert('Selection Required', 'Please select a letter type.', 'warning');
      return;
    }
  ...
  const handleViewPayslip = async () => {
    ...
    try {
      ...
      console.error(err);
      showAlert('Generation Error', 'Error generating payslip: ' + err.message, 'danger');
    } finally {
