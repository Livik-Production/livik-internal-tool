        ...
        if (res.ok) {
          ...
          if (uniqueActive.length > 0) {
            const getPaymentIcon = (label) => {
              ...
              if (lower.includes('petty') || lower.includes('cash') || lower.includes('💼')) return '💼';
              if (lower.includes('cash')) return '💰';
              if (lower.includes('cheque')) return '📄';
              if (lower.includes('wallet') || lower.includes('upi') || lower.includes('digital') || lower.includes('online')) return '📱';
              return '💸';
            };
            ...
            if (expenseData && (expenseData.paymentMode || expenseData.paymentMethod)) {
              const currentModeVal = expenseData.paymentMode || expenseData.paymentMethod;
              const hasCurrent = uniqueActive.some((item) => item.value === currentModeVal || item.label === currentModeVal);
              if (!hasCurrent) {
                mapped.push({
