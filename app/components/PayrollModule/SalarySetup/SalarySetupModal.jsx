    ...
    const fetchRules = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setSalaryRules({
            basicPayPercent: data.basicPayPercent !== undefined ? Number(data.basicPayPercent) : 40.0,
            hraPercent: data.hraPercent !== undefined ? Number(data.hraPercent) : 50.0,
          });
        }
      } catch (error) {
        console.error('Failed to fetch payroll rules from backend in SalarySetupModal:', error);
        ...
        if (saved) {
          try {
            ...
            setSalaryRules({
              basicPayPercent: parsed.basicPayPercent !== undefined ? Number(parsed.basicPayPercent) : 40.0,
              hraPercent: parsed.hraPercent !== undefined ? Number(parsed.hraPercent) : 50.0,
            });
          } catch (e) {
            console.error('Failed to load payroll rules in SalarySetupModal:', e);
          }
        }
      }
    };
                  ...
                  <p className="text-sm text-blue-800">
                    ...
                    {formData.ctc > 0 ? (
                      <>
                        ...
                        Basic Pay ({salaryRules.basicPayPercent}% of CTC), HRA ({salaryRules.hraPercent}% of Basic Pay), and
