...
export default function AssignRoleModal({
  ...
  return (
    <>
      ...
      >
        <div className="p-4">
        <form id="assignRoleForm" onSubmit={handleSubmit} className="space-y-4">
          {preselectedRole && (
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg">
              <p className="text-sm text-blue-700">
                <span className="font-semibold text-blue-800">Preselected Role:</span>{' '}
    ...
    <CustomAlertForm
      ...
      details={
        <div className="text-sm mt-2 p-2.5 bg-yellow-50 border border-yellow-100 rounded-lg text-yellow-800 leading-relaxed font-sans">
          <p className="mb-1"><span className="font-semibold text-yellow-900">Employee:</span> {formData.employeeName}</p>
          <p className="mb-1"><span className="font-semibold text-yellow-900">Current Role:</span> {formData.currentRole?.displayName || formData.currentRole?.roleName || 'Unknown'}</p>
          <p><span className="font-semibold text-yellow-900">New Role:</span> {(() => {
            const r = roles.find(role => {
              const val = typeof role === 'object' ? role.id || role.name || '' : role;
              return val === formData.role;
            });
            return typeof r === 'object' ? r.name || r.displayName || r.id : r || formData.role;
          })()}</p>
        </div>
      }
    />
