...
export default function TalentCommunityTab() {
  ...
  const exportToCSV = async () => {
    ...
    showToast('Preparing CSV with resume links...', 'success');
    ...
    try {
      const resolvedRows = await Promise.all(
        filtered.map(async (row) => {
          ...
          if (row.resume) {
            try {
              const res = await fetch(
                `/api/talent-community/download?key=${encodeURIComponent(row.resume)}`
              );
              const data = await res.json();
              if (res.ok && data.url) {
                resumeUrl = data.url;
              ...
              console.error('Error fetching signed URL for resume:', err);
              resumeUrl = row.resume;
            }
          }
      ...
      const csvRows = [
        ...
        ...resolvedRows.map((row) => {
          const dateSubmitted = row.createdAt
            ? new Date(row.createdAt).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
            ...
            : '';
          const experience =
            experienceLabelMap[row.experienceLevel] ||
            row.experienceLevel ||
            '';
      ...
      link.setAttribute(
        'download',
        `open_applications_${new Date().toISOString().split('T')[0]}.csv`
      );
