...
const LeaveRequestForm = ({
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (mounted && active.length > 0) {
            setDbLeaveTypes(active.map((item) => ({
              label: item.label,
              value: item.value,
            })));
          }
  ...
  const leaveTypes = React.useMemo(() => {
    ...
    if (initialData?.type || initialData?.leaveType) {
      const currentType = (initialData.type || initialData.leaveType).toLowerCase().replace(/-lop$/, '');
      const hasCurrent = list.some(item => item.value.toLowerCase() === currentType || item.label.toLowerCase() === currentType);
      if (!hasCurrent) {
        const label = currentType === 'sl' ? 'Sick Leave' : currentType === 'cl' ? 'Casual Leave' : currentType;
