...
export default function ProfileSetupWizard({ rawEmployeeData }) {
  ...
  useEffect(() => {
    ...
    const populateForm = (data) => {
      setForm((prev) => ({
        ...
        dateOfBirth:
          ...
          !data.dateOfBirth.startsWith('1970-01-01')
        ...
        dateOfJoining:
          data.dateOfJoining?.slice(0, 10) || prev.dateOfJoining || '',
    ...
    if (rawEmployeeData) {
      ...
      if (checkIsComplete(rawEmployeeData)) setIsSubmitted(true);
      ...
      if (rawEmployeeData.id && rawEmployeeData.id !== 'mock-123') {
        fetch(`/api/employees/${rawEmployeeData.id}`)
          ...
          .then((fullData) => {
            ...
            if (checkIsComplete(fullData)) setIsSubmitted(true);
          })
          .catch((err) =>
  ...
  const completionPercentage = React.useMemo(() => {
    ...
    let filledCount = fieldsToCheck.filter(
      (val) => val && String(val).trim() !== ''
    ).length;

    if (form.educationDetails && form.educationDetails.length > 0) {
      const hasValidEdu = form.educationDetails.some(
        (edu) => edu.institution || edu.qualification || edu.yearCompleted
      );
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'ifscCode' || field === 'panNumber') {
  ...
  const removeDegree = (idx) => {
    ...
    if (errors.educationDetails) {
      ...
      setErrors((p) => ({
        ...p,
        educationDetails: newEduErrors.length > 0 ? newEduErrors : undefined,
      }));
    }
  };
  ...
  const handleFileUpload = async (e, field) => {
    ...
    if (field === 'photo') {
      ...
      if (file.size > 300 * 1024) {
        showErrorToast('Profile photo must be less than 300KB');
        return;
      }
    }
  ...
  const validateStep = (s) => {
    ...
    if (s === 0) {
      ...
      } else if (!/^(?:\+?\d{1,4}[\s\-]*)?\d{10}$/.test(form.phoneNumber)) {
      ...
      } else if (!/^[A-Z]{4}0[A-Z0-9]{6}$/i.test(form.ifscCode)) {
      ...
      } else if (!/^[A-Z]{5}\d{4}[A-Z]$/i.test(form.panNumber)) {
        newErrors.panNumber = 'Invalid PAN format';
      }
      if (!form.photo) newErrors.photo = 'Profile photo is required';
      if (!form.aadhaarCard)
        newErrors.aadhaarCard = 'Aadhaar document is required';
    ...
    if (firstErrorKey) {
      ...
      if (firstErrorKey === 'educationDetails') {
        const firstErrIdx = newErrors.educationDetails.findIndex(
          (e) => e !== undefined
        );
  ...
  return (
    <div className="min-h-screen w-full bg-[#004475] text-sm font-sans flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-7xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] max-h-[900px]">
        ...
        {/* Right Panel */}
        {isSubmitted ? (
          <div className="w-full md:w-7/12 h-full flex items-center justify-center relative bg-slate-50/50 z-0 p-8">
            <div className="bg-white border border-gray-100 shadow-xl rounded-3xl p-10 max-w-[420px] w-full flex flex-col items-center text-center animate-in fade-in zoom-in duration-500">
              ...
              {currentStep === -1 && (
                <div className="flex flex-col mt-18 items-center justify-center min-h-[65vh] text-center px-4 animate-fade-in text-gray-800 font-bold">
                  <h1 className="text-4xl leading-snug mb-4">
                    Welcome to the Team, {form.firstName}
                  </h1>

                  <p className="text-[14px] max-w-md mx-auto mb-10 font-normal text-gray-500 leading-snug">
                    We're thrilled to have you here. Let's get your profile set
                    up and ready for your first day.
                  </p>
              ...
              {currentStep >= 0 && (
                <div className="flex justify-center mb-12">
                  <div id="photo" className="relative">
                    ...
                    <p className="absolute -bottom-9 left-1/2 -translate-x-1/2 text-[10px] text-gray-400 font-medium whitespace-nowrap">
                      ...
                      <div className="space-y-4">
                        {form.educationDetails.length === 0 ? (
                          ...
                          form.educationDetails.map((degree, idx) => (
                            <div
                              id={`educationDetails-${idx}`}
                              key={idx}
                              className="p-6 bg-gray-50/50 rounded-2xl border border-gray-200 relative group transition-all duration-300"
                            >
                              <button
                                type="button"
                    ...
                    <div data-index="3">
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                        <div className="col-span-2 p-4 bg-blue-50/50 border border-blue-100 rounded-xl flex items-start gap-3">
                          <Briefcase
                            size={16}
                            className="text-[#004475] shrink-0 mt-0.5"
                          />
                          <p className="text-[12px] text-blue-900 font-medium leading-relaxed">
                            Your employment information has been configured by
                            HR. If you see any discrepancies, please contact
