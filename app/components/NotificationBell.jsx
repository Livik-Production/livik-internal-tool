...
import { Bell, UserPlus, MessageSquare, TrendingUp, FileText } from 'lucide-react';
...
function NotificationBellContent({ placement = 'bottom' }) {
  ...
  }, [hiddenEnquiries, hiddenExpenseReport, readPendingIds, readLeaveIds, readDocIds]);

  useEffect(() => {
    const event = new CustomEvent('bell-state-change', { detail: { open: showNotifications } });
  ...
  const canViewDocuments = isSuperAdmin || hasRight('hr_module_control_all_employees') || hasRight('hr_module_view_all_employees');
  const isTrueSuperAdmin =
    roleName === 'SUPER_ADMIN' ||
    roleName === 'SUPER ADMIN' ||
    roleName === 'SUPERADMIN';
    
  let docRoleFilter = 'EMPLOYEE';
  if (isTrueSuperAdmin) {
    docRoleFilter = 'SUPER_ADMIN,ADMIN,HR_ADMIN,HR,EMPLOYEE';
  } else if (roleName === 'ADMIN') {
    docRoleFilter = 'HR_ADMIN,HR,EMPLOYEE';
  } else if (roleName === 'HR_ADMIN') {
    docRoleFilter = 'HR,EMPLOYEE';
  }
  
  const docSWRKey = canViewDocuments ? `/api/document-requests?status=PENDING&requestedByRole=${docRoleFilter}` : null;
  const { data: pendingDocsData } = useSWR(docSWRKey, fetcher, { refreshInterval: 60000 });
  ...
  const notifications = [...mappedDbNotifs, ...pendingNotifs, ...pendingLeaveNotifs, ...pendingDocNotifs, ...enquiryNotifs, ...expenseNotifs].filter((n) => !n.isRead);
  ...
  const handleNotificationClick = async (notif) => {
    ...
    if (notif.id && !notif.id.toString().startsWith('pending-') && !notif.id.toString().startsWith('doc-') && !notif.id.toString().startsWith('enquiry-') && notif.id !== 'weekly-expense-report') {
      if (!notif.isRead) {
        try {
