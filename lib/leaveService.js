...
import { sendLeaveRequestNotification, sendLeaveStatusNotification } from './mailer';
  ...
  return safeExecute(async (prisma) => {
    ...
    try {
      const [employee, admins] = await Promise.all([
        ...
        prisma.employee.findMany({
          where: {
            role: {
              roleName: {
                in: ['hr_admin', 'admin', 'super_admin', 'HR_ADMIN', 'ADMIN', 'SUPER_ADMIN']
              }
            },
            email: { not: null }
          },
      ...
      if (adminEmails.length > 0) {
        // use static import defined at the top
        const empName = employee ? `${employee.firstName || ''} ${employee.lastName || ''}`.trim() : 'An employee';
  ...
  return safeExecute(async (prisma) => {
    ...
    if (req.leaveType === 'LOP') {
      ...
      await NotificationService.createBulkNotifications([req.employeeId], {
        ...
        message: `Your leave request for ${req.totalDays || 0} day(s) from ${new Date(req.startDate).toLocaleDateString()} to ${new Date(req.endDate).toLocaleDateString()} has been approved.`,
        type: 'LEAVE',
      });
      ...
      if (applicant.email) {
        try {
          const applicantName = `${applicant.firstName || ''} ${applicant.lastName || ''}`.trim() || 'Employee';
          const approverNameStr = `${approver.firstName || ''} ${approver.lastName || ''}`.trim() || 'Approver';
...
export async function rejectLeave(id, approverId, remarks = null) {
  ...
  return safeExecute(async (prisma) => {
    ...
    await NotificationService.createBulkNotifications([req.employeeId], {
      ...
      message: `Your leave request for ${req.totalDays || 0} day(s) from ${new Date(req.startDate).toLocaleDateString()} to ${new Date(req.endDate).toLocaleDateString()} has been rejected.`,
      type: 'LEAVE',
    });
