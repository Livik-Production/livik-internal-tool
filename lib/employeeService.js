...
export async function generateContractEmpId(year, pad = 3) {
  ...
  try {
    formatSetting = await safeExecute((pc) =>
      pc.numberFormatSetting.findUnique({
        where: { module: 'contract_employee' },
      })
    );
  } catch (err) {
  ...
  if (!seq || Number.isNaN(seq)) {
    throw new Error(
      'Failed to get sequence value (contract_employee_number_seq).'
    );
  }
  return `${effectivePrefix}${String(seq).padStart(effectivePad, '0')}${effectiveSuffix}`;
}
...
export async function updateEmployee(id, data) {
  ...
  if ('photo' in data) {
    ...
    const isSignedS3Url =
      val &&
      typeof val === 'string' &&
      (val.includes('X-Amz-') || val.includes('amazonaws.com'));
    if (!isSignedS3Url) {
  ...
  if (
    'workType' in data &&
    String(data.workType).toUpperCase() === 'CONTRACT'
  ) {
    const existing = await safeExecute((pc) =>
      pc.employee.findFirst({
