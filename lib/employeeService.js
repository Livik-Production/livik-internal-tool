...
export async function updateEmployee(id, data) {
  ...
  if ('aadhaarCard' in data)
    employeeData.aadhaarCard = data.aadhaarCard ?? null;
  if ('panCard' in data) employeeData.panCard = data.panCard ?? null;
  ...
  if ('totalExperience' in data) employeeData.totalExperience = data.totalExperience ?? null;
  if ('projectsDone' in data) employeeData.projectsDone = data.projectsDone ?? null;
 ...
 */
export async function checkRoleAssignmentPermission(requesterId, targetEmployeeId, newRoleId) {
  if (!requesterId) {
    throw new Error('Requester ID is required for role assignment authorization.');
  }
  ...
  return safeExecute(async (prisma) => {
    ...
    if (targetEmployeeId) {
      ...
      const currentTargetRole = targetEmployee?.role?.roleName?.toUpperCase() || null;

      // An ADMIN cannot modify the role of another ADMIN or a SUPER_ADMIN
      if (currentTargetRole === 'SUPER_ADMIN' || currentTargetRole === 'ADMIN') {
        throw new Error('Forbidden: Only Super Admin can modify the role of an Admin or Super Admin.');
      }
    }
    ...
    if (newRoleId) {
      ...
      if (newRoleName === 'SUPER_ADMIN' || newRoleName === 'ADMIN') {
        throw new Error('Forbidden: Only Super Admin can assign Admin or Super Admin roles.');
      }
    }
