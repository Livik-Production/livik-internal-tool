...
export async function addInvoicePayment(invoiceId, data) {
  ...
  return safeExecute(async (prisma) => {
    ...
    const totalPaidPayments = invoice.payments.reduce(
      (sum, p) => sum + Number(p.receivedAmount),
      0
    );
    const totalPaidPartial = invoice.partialPayments.reduce(
      (sum, p) => sum + Number(p.receivedAmount),
      0
    );
    const totalPaid =
      totalPaidPayments + totalPaidPartial + Number(data.receivedAmount);
      ...
      payment = await prisma.invoicePartialPayment.create({
        data: {
          ...
          nextInstallmentDate: data.nextInstallmentDate
            ? toDate(data.nextInstallmentDate)
            : null,
        },
      });
    } else {
