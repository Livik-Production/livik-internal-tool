...
async function enrichInvoice(inv, prisma) {
  ...
  const totalPaidPartial =
    inv.partialPayments?.reduce((sum, p) => sum + Number(p.receivedAmount || 0), 0) ||
    0;
  const totalPaid = totalPaidPayments + totalPaidPartial;
  ...
  const firstPaymentMethod = inv.payments?.[0]?.paymentMethod || inv.partialPayments?.[0]?.paymentMethod || '';
  const firstPaymentDate = inv.payments?.[0]?.paymentDate || inv.partialPayments?.[0]?.paymentDate || null;
  ...
  return {
    ...
    // Explicitly mapped for PreviewForm Tax Table calculations
...
export async function createInvoice(data) {
  ...
  return safeExecute((prisma) =>
    prisma.invoice.create({
      ...
      include: { items: true, payments: true, partialPayments: true, followUps: true },
    })
  );
}
...
export async function getInvoiceById(id) {
  ...
  return safeExecute(async (prisma) => {
    const invoice = await prisma.invoice.findUnique({
      where: { id },
      include: { items: true, payments: true, partialPayments: true, followUps: { orderBy: { date: 'desc' } } },
    });
...
export async function deleteInvoice(id) {
  ...
  return safeExecute(async (prisma) => {
    ...
    await prisma.invoicePartialPayment.deleteMany({
      where: { invoiceId: id },
    });
