...
export async function getAllPermissionRequests(filters = {}) {
  return safeExecute(async (prisma) => {
    ...
    if (resolvedEmployeeId && !resolvedEmployeeId.startsWith('c') && !resolvedEmployeeId.includes('-')) {
      const emp = await prisma.employee.findFirst({
        where: { OR: [{ id: resolvedEmployeeId }, { empId: resolvedEmployeeId }] },
      });
      if (emp) resolvedEmployeeId = emp.id;
    }

    let resolvedExcludeId = filters.excludeEmployeeId;
    if (resolvedExcludeId && !resolvedExcludeId.startsWith('c') && !resolvedExcludeId.includes('-')) {
      const emp = await prisma.employee.findFirst({
        where: { OR: [{ id: resolvedExcludeId }, { empId: resolvedExcludeId }] },
      });
...
function requiredApproverRoleFor(applicantRoleName = '') {
  ...
  return ['HR', 'ADMIN', 'HR ADMIN', 'SUPER_ADMIN'];
}
...
export async function approvePermissionRequest(id, approverId, remarks = null) {
  ...
  return safeExecute(async (prisma) => {
    ...
    if (approver.id === applicant.id && applicantRole !== 'SUPER_ADMIN') {
