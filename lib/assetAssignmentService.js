...
export const assetAssignmentService = {
  assignAsset: async (assetId, employeeId, assignedDate, notes, assignmentType = 'EMPLOYEE', locationId = null) => {
    ...
    if (assignmentType === 'EMPLOYEE' && employeeId) {
      await NotificationService.createBulkNotifications([employeeId], {
        ...
        type: 'ASSET',
      }).catch(err => console.error('Failed to notify employee of asset assignment', err));
    }
